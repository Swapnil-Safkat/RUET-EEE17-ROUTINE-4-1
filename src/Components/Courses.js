...
const Courses = () => {
  ...
  let buttonColorD = "bg-secondary";
  let buttonColor = "bg-primary opacity-20";
  useEffect(() => {
      ...
      <div className='w-full lg:w-1/5 '>
        <div className='w-full flex flex-col'>
          ...
          <div className='flex flex-row w-full justify-between my-2'>
            ...
            } className={`w-1/2 shadow-lg shadow-slate-600/20 mx-1 font-bold text-gray-200 border-2 border-violet-500 ${section[0] ? buttonColor : buttonColorD} rounded-lg px-4 py-2`}>A</button>

            <button disabled={!section[0]} onClick={() => { setSection([false,"B"]) }
            } className={`w-1/2 shadow-lg shadow-slate-600/20 mx-1 font-bold text-gray-200 border-2 border-violet-500 ${!section[0] ? buttonColor : buttonColorD} rounded-lg px-4 py-2`}>B</button>
                          ...
                          cls.map((c) => {
                            return <td className='bg-zinc-900/20 rounded-md text-center'>
                              {c === "x" || !selectCourses.includes(c.course) ?
                                ...
                                <div className='flex flex-col tracking-wide'>
                                  <h1 className='text-sm font-bold tracking-widest'>{c.short}</h1>
                            ...
                            <td className='bg-zinc-800/20 text-center tracking-wide'>
                              {selectCourses.includes(cls[0].course) ? <div className='flex flex-col'>
                                <h1 className='text-sm font-bold tracking-widest'>{cls[0].short}</h1>
                                <div className='h-[2px] bg-green-500/20' />
